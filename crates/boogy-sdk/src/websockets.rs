...
//!
//! ```ignore
//! // `PublishError` / `GrantError` do NOT convert into `ApiError`, so a bare
//! // `?` will not compile in a handler — render or match them.
//! fn tick() -> Result<String, ApiError> {
//!     ws_publish("ticker", &json::json!({"px": 42}).to_string())
