...
**The two exceptions, and they travel together:** `Api` (the trait you
implement) and `Schema` (its `schema` method's parameter type) are NOT
injected, so `lib.rs` always carries
`use boogy_sdk::{schema_decl::Schema, Api};`. The failure is asymmetric and
that is why it is called out here: forgetting an injected name gets you a
loud `E0252` telling you to delete the line, while forgetting *this* one
gets you `cannot find type Schema in this scope` on a line where every
other name resolved — which reads like the macro half-worked. (`smoke/`
implements only `build_router`, so the template never shows it.)

And `wit_glue!(bindings, MyApi)` does **not** define
