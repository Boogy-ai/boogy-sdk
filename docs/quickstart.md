...
```rust ignore-snippet: a build.rs. boogy-wit is a build-dependency, so boogy_wit::wit_dir() is not in scope in a lib target — this file only compiles as a build script.
// build.rs
use std::fs;
use std::path::Path;
