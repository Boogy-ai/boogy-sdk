...
The host parses and validates the manifest on every deploy. **Unknown fields are rejected everywhere**, in every section — a misspelled or stale key is a hard parse failure at deploy time, not a silently ignored line. That includes `[capabilities]`: a capability name that does not exist fails the deploy rather than defaulting to off. Stick to documented fields.
